...
<div class={`grid w-full items-center mt-5 ${align === 'left' ? 'grid-cols-[auto_1fr_auto]' : 'grid-cols-[1fr_auto_1fr]'}`}>
	<div class={`flex items-center ${align === 'left' ? 'justify-start col-start-1' : 'justify-center col-start-2'}`}>
		<div class="mr-2">
			<input class="hidden" id={`flip-order-${idPrefix}`} type="checkbox" bind:checked={descending} />
		...
		<ConnectedButtons>
			{#each sortOptions as option}
				<input id={`sort-${idPrefix}-${option.key}`} type="radio" name={`sortKey-${idPrefix}`} value={option.key} bind:group={sortKey} />
	...
	<div class={`flex justify-end mr-10 ${align === 'left' ? 'col-start-3' : 'col-start-3'}`}>
		<SettingsGear />
	</div>
</div>
