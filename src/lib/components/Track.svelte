...
{#if showMenu}
	<div
		...
		class="scale-100 transform opacity-100 transition-all duration-100"
	>
		<div class="bg-surface-container border-outline min-w-48 rounded-xl border py-2 shadow-lg">
			<button
			...
			{#if type === 'playlist'}
				...
				>
					...
					>
						<path
							fill="currentColor"
							d="M7 17q-.825 0-1.412-.587T5 15V5q0-.825.588-1.412T7 3h8q.825 0 1.413.588T17 5v2h-2V5H7v10h2v2zm4 4q-.825 0-1.412-.587T9 19V9q0-.825.588-1.412T11 7h8q.825 0 1.413.588T21 9v10q0 .825-.587 1.413T19 21zm0-2h8V9h-8z"
						/>
					</svg>
				...
				<div class="relative">
					...
					{#if showPlaylistSubmenu}
						<div
							...
							onmouseenter={handlePlaylistHover}
							onmouseleave={handlePlaylistLeave}
							class="bg-surface-container border-outline absolute top-0 left-full z-50 ml-1 min-w-40 rounded-xl border py-2 shadow-lg"
						>
...
<div class="mt-4 ml-4 flex w-full min-w-0 flex-col md:mt-5 md:ml-0" oncontextmenu={rightClickContextMenu}>
	{#if track?.image || playlist?.image || album?.image || artist?.image}
			...
			<Lazy height={200} keep={true}>
				{#if type === 'artist' && USE_ARTIST_SVG}
					...
					<svg
						...
						xmlns="http://www.w3.org/2000/svg"
						viewBox="0 0 24 24"
						class="text-primary bg-primary-container h-44 w-44 cursor-pointer rounded-lg object-cover transition-all duration-300 ease-in-out select-none hover:scale-102 hover:shadow-lg active:scale-98 md:aspect-square md:h-auto md:w-full md:max-w-60 lg:max-w-64"
						><path
							fill="currentColor"
				...
				{:else}
					...
					<img
						...
						class="h-44 w-44 cursor-pointer rounded-lg object-cover transition-all duration-300 ease-in-out select-none hover:scale-102 hover:shadow-lg active:scale-98 md:aspect-square md:h-auto md:w-full md:max-w-60 lg:max-w-64"
		...
		{/await}
	{:else if type === 'artist' && USE_ARTIST_SVG}
		<!-- svelte-ignore a11y_click_events_have_key_events -->
		<svg
			onclick={handleTrackClick}
			xmlns="http://www.w3.org/2000/svg"
	...
	{:else}
		...
		/>
	{/if}
	<div class="flex w-44 flex-col select-none md:w-full md:max-w-60 lg:max-w-64">
		<p
