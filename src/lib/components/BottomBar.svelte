...
<div class="relative flex flex-col h-[98%] items-center">
	<div class="flex flex-col ml-2 sm:ml-0">
		{#if $activeSong.title}
			...
			{:catch error}
				<img
					src="/temp/MapleD.svg"
					alt={$activeSong.title}
					class="h-96 mt-3 self-center rounded-xl p-2"
				/>
			{/await}
		...
		{:else}
		<img src="/temp/MapleD.svg" alt="gnx" class="h-96 mt-3 self-center rounded-[5%] p-2" />
		<div class="flex-col items-start flex">
			<Button variant="link" class="mt-5 ml-5 font-bold text-xl mb-0 p-0 h-fit underline-offset-1 py-0 px-0">Nothing Playing</Button>
			<Button variant="link" class="mt-[-0.5rem] ml-5 p-0 h-3 text-mc font-normal underline-offset-1 py-0 px-0 my-0">...</Button>
		</div>
		{/if}
	</div>
	<div class="w-[90%] h-fit mt-6 ml-1 bottom-64 flex items-center justify-center">
		<!-- svelte-ignore a11y-missing-attribute -->
		<!-- svelte-ignore a11y-no-static-element-interactions -->
		<a class="w-full h-full" on:click={() => handleScrub($curTime[0])} on:mouseup={() => handleScrub($curTime[0])}>
			<Slider
				color="bg-primary"
				bind:value={$curTime}
				bind:max={maxDuration[0]}
	...
	<div class="absolute left-1/2 bottom-52 flex -translate-x-1/2 transform items-center justify-center">
		<Button
			disabled
			class="flex h-fit w-fit flex-row items-center justify-start bg-transparent p-2 hover:bg-secondary"
		>
	...
	<div class="mr-5 flex-grow justify-end flex">
		<!-- svelte-ignore a11y-no-static-element-interactions -->
		<!-- svelte-ignore a11y-missing-attribute -->
	...
	<div class="mr-1 bottom-0 justify-end flex mb-2">
		<Button
				on:click={drawerUp}
				variant="secondary"
				class="flex h-fit w-fit flex-row items-center justify-start p-2 rounded-[50%]"
