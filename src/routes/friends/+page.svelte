	...
	<div class="flex w-full max-w-4xl flex-col gap-8">
		{#if !$isLoggedIn}
			<div class="flex flex-col items-center justify-center gap-4 py-16">
				<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" class="text-on-surface-variant">
		...
		{:else}
			...
			<section class="flex flex-col gap-3">
				...
				<Card variant="outlined" class="p-6">
					<div class="flex flex-col gap-4 sm:flex-row sm:items-center">
						...
						/>
						<Button variant="filled" onclick={addFriend} disabled={addingFriend || !selectedFriend.trim()}>
							{#if addingFriend}
								Sending...
							{:else}
			...
			{#if $pendingRequests && $pendingRequests.length > 0}
				<section class="flex flex-col gap-3">
					<div class="flex flex-col gap-1">
						<p class="text-on-surface-variant text-xs tracking-[0.16em] uppercase">Friend Requests</p>
						...
						<div class="flex flex-col gap-4">
							{#each $pendingRequests as request}
								{#await UserManager.getUserbyId(request.user_id) then user}
									{#if user}
										<div class="bg-surface-container-high flex flex-col gap-4 rounded-xl p-4 sm:flex-row sm:items-center sm:justify-between">
											<div class="flex items-center gap-4">
												{#if user.pfp}
													<img src={user.pfp} alt="Profile" class="h-12 w-12 rounded-full object-cover" />
												{:else}
												...
												{/if}
												<div>
													<h3 class="text-on-surface font-semibold">{user.name || user.username}</h3>
											...
											<div class="flex items-center gap-2">
												<Button variant="filled" onclick={() => acceptRequest(request.user_id)}>
													<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" class="mr-1">
														<path fill="currentColor" d="m9 16.2l-3.5-3.5a.984.984 0 0 0-1.4 0a.984.984 0 0 0 0 1.4l4.19 4.19c.39.39 1.02.39 1.41 0L20.3 7.7a.984.984 0 0 0 0-1.4a.984.984 0 0 0-1.4 0z"/>
													</svg>
													Accept
												</Button>
				...
				<Card variant="outlined" class="p-6">
							...
							{#each $friends as friend}
								<div class="bg-surface-container-high flex flex-col gap-4 rounded-xl p-4 sm:flex-row sm:items-center sm:justify-between">
									<div class="flex min-w-0 items-center gap-4">
										{#if friend.pfp}
											<img src={friend.pfp} alt="Profile" class="h-12 w-12 flex-shrink-0 rounded-full object-cover" />
										{:else}
											<div class="bg-primary/20 text-primary flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full">
												<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
													<path fill="currentColor" d="M12 12q-1.65 0-2.825-1.175T8 8t1.175-2.825T12 4t2.825 1.175T16 8t-1.175 2.825T12 12m-8 8v-2.8q0-.85.438-1.562T5.6 14.55q1.55-.775 3.15-1.162T12 13t3.25.388t3.15 1.162q.725.375 1.163 1.088T20 17.2V20z"/>
												</svg>
											</div>
										...
										<div class="min-w-0 flex-1">
											<h3 class="text-on-surface truncate font-semibold">{friend.name || friend.username}</h3>
											<p class="text-on-surface-variant truncate text-sm">@{friend.username}</p>
											{#if friend.nowPlaying}
												<div class="mt-1 flex items-center gap-2">
													<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" class="text-primary flex-shrink-0">
														<path fill="currentColor" d="M12 5v8.55c-.94-.54-2.1-.75-3.33-.32c-1.34.48-2.37 1.67-2.61 3.07a4.007 4.007 0 0 0 4.59 4.65c1.96-.31 3.35-2.11 3.35-4.1V7h2c1.1 0 2-.9 2-2s-.9-2-2-2h-2c-1.1 0-2 .9-2 2"/>
													</svg>
					...
					{:else}
						<div class="flex flex-col items-center justify-center gap-2 py-8">
							<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 640 512" class="text-on-surface-variant">
