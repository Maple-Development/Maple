<script lang="ts">
    ...
    function handleAlbumFiltersChange(payload: { sorted: Album[]; sortKey: string; descending: boolean }) {
        sortedAlbums = payload.sorted;
        albumSortKey = payload.sortKey as AlbumSortKey;
        albumDescending = payload.descending;
    }

    function handleTrackFiltersChange(payload: { sorted: Song[]; sortKey: string; descending: boolean }) {
