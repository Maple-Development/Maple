...
<div
	class="mr-5 mb-10 ml-2 grid grid-cols-2 gap-x-2 gap-y-2 sm:gap-x-3 md:mx-8 md:grid-cols-3 md:gap-x-4 lg:mx-16 lg:grid-cols-4 lg:gap-x-6 xl:grid-cols-5"
>
	<div class="mt-4 ml-4 flex w-full min-w-0 flex-col md:mt-5 md:ml-0">
		<!-- svelte-ignore a11y_no_static_element_interactions -->
		<!-- svelte-ignore a11y_click_events_have_key_events -->
		<div
			onclick={createPlaylist}
			class="bg-primary-container text-primary flash-slow flex h-44 w-44 cursor-pointer items-center justify-center rounded-lg transition-all duration-300 ease-in-out select-none hover:scale-102 hover:shadow-lg active:scale-98 md:aspect-square md:h-auto md:w-full md:max-w-60 lg:max-w-64"
		>
