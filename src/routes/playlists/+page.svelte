<script lang="ts">
    ...
    function handleFiltersChange(payload: { sorted: Playlist[]; sortKey: string; descending: boolean }) {
...
<div class="grid grid-cols-1 mx-24 gap-x-2 gap-y-2 sm:grid-cols-2 sm:gap-x-3 md:mx-16 md:grid-cols-3 md:gap-x-4 lg:grid-cols-4 lg:gap-x-2 xl:grid-cols-5 xl:gap-x-2">
    <div class="flex flex-col w-full mt-5 ml-10">
        <!-- svelte-ignore a11y_no_static_element_interactions -->
        <!-- svelte-ignore a11y_click_events_have_key_events -->
        <div
            onclick={createPlaylist}
            class="h-44 w-44 md:h-52 md:w-52 rounded-lg cursor-pointer transition-all duration-300 ease-in-out hover:scale-102 hover:shadow-lg active:scale-98 select-none bg-primary-container text-primary flex items-center justify-center flash-slow"
        >
    ...
    {#each sortedPlaylists as playlist (playlist.id)}
        <div class="will-change-transform" animate:flip={{ duration: 300, easing: cubicOut }}>
            <Track playlist={playlist} type="playlist" />
        </div>
    {/each}
</div>
